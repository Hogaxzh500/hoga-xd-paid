...
THIS TOOL PAID 
 ...
 [\x1b[1;96m+%s] Author   : HOGA XZH - XD 

 [\x1b[1;96m+%s] Github   : github.com/Hogaxzh500

 [\x1b[1;95m+%s] Facebook  Mau. HO GA XZH 

 [\x1b[1;94m+%s] This TOOL WAS PAID ( PAID TOOL) : [ ,Xl,3]
...
class login:
	...
	def __login__(self):
		...
		token = input(' [%s*%s] Need token : '%(O,M))
	...
	def bot(self):
		...
		try:
			...
			jalan('\n [%s!%s] Token success !'%(B,O));time.sleep(1);login().__login__()
...
class menu:
	...
	def main(self):
		...
		jalan(' %s[ %sHOGA-XD %s%s ]'%(M,H,K,B))

		print(' %s[%s•%s] IP  : %s'%(N,O,N,IP))

		print(' %s[%s•%s] DATE :        : %s'%(H,O,H,waktu))

		print(' %s'%(M))

		print(' %s[%s0%s] friend List crack '%(M,O,M))

		print(' %s[%s1%s] public id Crack '%(M,O,M))

		print(' %s[%s2%s] Coming soon  [error]'%(M,O,M))

		print(' %s[%s3%s] Coming soon [error]'%(M,O,M))

		print(' %s[%s4%s] like post Crack '%(M,O,M))

		print(' %s[%s5%s] Followrs crack '%(M,O,M))

		print(' %s[%s6%s]  Coming soon [error]'%(N,O,N))

		print(' %s[%s7%s]  check cp and ok '%(N,O,N))

		print(' %s[%s8%s]  comintern soon [error'%(N,O,N))

		print(' %s[%s9%s] crack email'%(N,O,N))

		print(' %s[%s10%s] [error]'%(N,O,N))

		print(' %s[%s11%s] [error]'%(N,O,N))

		print(' %s[%s00%s]  close token'%(N,O,N))
...
class crack:
	...
	def fbeh(self,id):
		...
		if kham in ['']:
			...
			print(' %s [%s3%s] Metode Mobile/RECOMEND%(N,O,N))
	...
	def free(self):
		...
		print(' %s [%s*%s] ok ids  saved  file  : hogaxd/okeh.txt'%(N,O,N))

		print(' %s [%s*%s] cp ida saved file    : hogaxd/cepeh.txt\n'%(N,O,N))
